...
class _AdvancedAnalyticsScreenState extends State<AdvancedAnalyticsScreen>
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Icon(Icons.analytics, size: 32, color: AppColors.primary),
          ),
        ],
      ),
    );
  ...
  Widget _buildWeeklyLearningChart(
    BuildContext context,
                  ...
                  LineChartBarData(
                    spots:
                        weeklyData.asMap().entries.map((entry) {
                          return FlSpot(
                            entry.key.toDouble(),
                            (entry.value['minutesSpent'] as int).toDouble(),
                          );
                        }).toList(),
                    isCurved: true,
                    color: AppColors.primary,
  ...
  Widget _buildSkillCompletionTrends(
    BuildContext context,
    AnalyticsService analyticsService,
  ) {
            ...
            child: LineChart(
              LineChartData(
                ...
                lineBarsData: [
                  LineChartBarData(
                    spots:
                        trendsData.asMap().entries.map((entry) {
                          return FlSpot(
                            entry.key.toDouble(),
                            (entry.value['completedSkills'] as int).toDouble(),
                          );
                        }).toList(),
                    isCurved: true,
  ...
  Widget _buildLearningActivityDetails(
    BuildContext context,
    AnalyticsService analyticsService,
  ) {
    final weeklyData = analyticsService.getWeeklyLearningActivity();
          ...
          SizedBox(
            ...
            child: BarChart(
              BarChartData(
                ...
                borderData: FlBorderData(show: false),
                barGroups:
                    progress.asMap().entries.map((entry) {
  ...
  Widget _buildIndividualGoalProgress(
    BuildContext context,
    List<Map<String, dynamic>> progress,
  ) {
    return Container(
  ...
  Widget _buildCareerGoalsProgress(
    BuildContext context,
    AnalyticsService analyticsService,
  ) {
    final progress = analyticsService.getCareerGoalsProgress();
  ...
  void _showExportDialog(BuildContext context) {
    showDialog(context: context, builder: (context) => PDFExportWidget());
  }
